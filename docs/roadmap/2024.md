...
* [ ]  取消 db 实体，探索如何使用一个 orm 无关的实体，也能让 orm 功能生效。可能需要使用 lombok 自定义注解
...
* [ ]  Long类型的日期时间字段，字段以 Date 或 Time 结尾时，实体添加相关获取时间对象的方法和设置方法，方便业务调用
  - 参考Lombok，创建自定义注解，避免每次更新框架功能都需要重新覆盖生成的实体类
  - 例如有 createTime 的 Long 字段，则自动添加 ldtOfCreateTime() 、ldOfCreateTime()、ltOfCreateTime()、setCreateTimeByLdt()、setCreateTimeByLd()、setCreateTimeByLt()方法
