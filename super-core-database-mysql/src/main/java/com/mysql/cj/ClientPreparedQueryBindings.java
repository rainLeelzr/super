...
/**
 * 修改 setString 方法，让其支持在使用 mybatis-plus 不写 xml 时，能正确 insert 空间字段
 */
public class ClientPreparedQueryBindings extends AbstractQueryBindings<ClientPreparedQueryBindValue> {
    ...
    public void setString(int parameterIndex, String x) {
        if (x == null) {
            ...
            // 支持空间字段
            if (parameterAsString.startsWith("geomerty:")) {
                needsQuoted = false;
            }
            ...
            // 支持空间字段
            if (parameterAsString.startsWith("geomerty:")) {
                parameterAsString = parameterAsString.replace("geomerty:", "");
            }
